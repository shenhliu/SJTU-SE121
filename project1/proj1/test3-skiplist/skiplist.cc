...
int SkipList::clear()
{
    ...
    while(topHeader->below)
    {
        ...
        else
        {
            ...
            // invalidNode.erase(p);
            // invalidNode.erase(q);
            // invalidNode.erase(topHeader);
            // invalidNode.erase(topTrailer);
            // topHeader->below = p->below;
            // topTrailer->below = q->below;
            // if(p->below && q->below)
            // {
            //     p->below->above = topHeader;
            //     q->below->above = topTrailer;
            // }
            // delete p;
            // delete q;
            // _level--;
...
QuadListNode* SkipList::insertAsFirst(uint64_t k,const std::string &v)
{
    ...
    while(rand() & 1)
    {
        // QuadListNode* p = new QuadListNode(k,v,topHeader,topTrailer,NULL,topHeader->below->succ);
        // QuadListNode* q = new QuadListNode(0," ",NULL,NULL,NULL,topHeader);
        // QuadListNode* r = new QuadListNode(0," ",NULL,NULL,NULL,topTrailer);
        // invalidNode.insert(q);
        // invalidNode.insert(r);
        // topHeader->below->succ->above = p;
        // topHeader->succ = p;
        // topHeader->above = q;
        // topTrailer->above = r;
        // topTrailer->pred = p;
    ...
    while(rand() & 1)
    {
        ...
        if(level != topHeader)
        {
            ...
            else
            {
                ...
                {
                    while(valid(q) && (q->key <= k))    {q = q->succ;}
                    q = q->pred;
                }                
                p = q;
                p->insertAsSuccAbove(k,v,'V',current);
        ...
        {
            // _level++;
            // topHeader->above = new QuadListNode(0," ",NULL,NULL,NULL,topHeader);
            // topTrailer->above = new QuadListNode(0," ",NULL,NULL,NULL,topTrailer);
            // topHeader = topHeader->above;
            // topTrailer = topTrailer->above;
            // invalidNode.insert(topHeader);
            // invalidNode.insert(topTrailer);
            // topHeader->succ = topTrailer;
            // topTrailer->pred = topHeader;
            // level->insertAsSuccAbove(k,v,current);
            // current = level->succ;
            increaseLevel();
            topHeader->below->insertAsSuccAbove(k,v,'V',current);
            current = topFirst();
